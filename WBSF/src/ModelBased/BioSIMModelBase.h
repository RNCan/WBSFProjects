...
public:
    ...
    {
        return m_info.m_outputFilePath;
    }

    //Simulated Annealing functionality
    virtual void AddResult(const std::vector<std::string>& header, const std::vector<std::string>& data);
    ...
    virtual void AddAnnualResult(const std::vector<std::string>& header, const std::vector<std::string>& data) {}
    virtual void AddAtemporalResult(const std::vector<std::string>& header, const std::vector<std::string>& data) {}
    ...
    virtual bool GetFValueAtemporal(CStatisticXY& stat)
    {
        assert(false);
        return true;
    }
    ...
    void SetOutput(CModelStatVector& in)
    {
        m_output.swap(in);
    }
    const CModelStatVector& GetOutput()const
    {
        return m_output;
    }
    ...
    const CTransferInfoIn& GetInfo()const
    {
        return m_info;
    }
    ...
    const CSAResultVector& GetSAResult()const
    {
        return m_SAResult;
    }

    void InitRandomGenerator(size_t seed)
    {
        m_randomGenerator.Randomize(seed);
    }
