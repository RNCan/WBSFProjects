...
ERMsg CBioSIMModelBase::Execute()
{
    ...
    try
    {
        ...
        case CTM::DAILY:
            ...
            break;
        case CTM::MONTHLY:
            msg = OnExecuteMonthly();
            break;
        case CTM::ANNUAL:
            msg = OnExecuteAnnual();
            break;
        case CTM::ATEMPORAL:
            msg = OnExecuteAtemporal();
            break;
        default:
            msg = GetErrorMessage(ERROR_REF_NOT_SUPPORT);
        }
    }
    catch (ERMsg e)
    {
...
ERMsg CBioSIMModelBase::OnExecuteHourly()
{
    return GetErrorMessage(ERROR_REF_NOT_SUPPORT);
}
ERMsg CBioSIMModelBase::OnExecuteDaily()
{
    return GetErrorMessage(ERROR_REF_NOT_SUPPORT);
}
ERMsg CBioSIMModelBase::OnExecuteMonthly()
{
    return GetErrorMessage(ERROR_REF_NOT_SUPPORT);
}
ERMsg CBioSIMModelBase::OnExecuteAnnual()
{
    return GetErrorMessage(ERROR_REF_NOT_SUPPORT);
}
ERMsg CBioSIMModelBase::OnExecuteAtemporal()
{
    return GetErrorMessage(ERROR_REF_NOT_SUPPORT);
}
...
void CBioSIMModelBase::AddResult(const std::vector<std::string>& header, const std::vector<std::string>& data)
{
    switch (m_info.m_TM.Type())
    {
    case CTM::HOURLY:
        ...
        break;
    case CTM::DAILY:
        AddDailyResult(header, data);
        break;
    case CTM::MONTHLY:
        AddMonthlyResult(header, data);
        break;
    case CTM::ANNUAL:
        AddAnnualResult(header, data);
        break;
    case CTM::ATEMPORAL:
        AddAtemporalResult(header, data);
        break;
    default:
        assert(false);
    }
...
bool CBioSIMModelBase::GetFValue(const std::vector<double>& paramArray, CStatisticXY& stat)
{
    ...
    for (size_t i = 0; i < m_info.m_repCounter.GetTotal()&& bValid; i++)
    {
        ...
        {
        case CTM::HOURLY:
            bValid &= GetFValueHourly(statTmp);
            break;
        case CTM::DAILY:
            bValid &= GetFValueDaily(statTmp);
            break;
        case CTM::MONTHLY:
            bValid &= GetFValueMonthly(statTmp);
            break;
        case CTM::ANNUAL:
            bValid &= GetFValueAnnual(statTmp);
            break;
        case CTM::ATEMPORAL:
            bValid &= GetFValueAtemporal(statTmp);
            break;
        default:
            assert(false);
        }
...
{
    assert(m_output.GetFirstTRef().TM() == m_info.m_TM);
