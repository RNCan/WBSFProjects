...
namespace WBSF
{
	...
	*/
	...
	{
		//std::vector<std::string> list = Tokenize(m_listValues, ";|\r\n|");
		return m_type == kMVListByCSV;// && list.size() == 4 && list[0] == "FROM_CSV_FILE";

	}

	std::vector<std::string> CModelInputParameterDef::GetList(const string& appPath, const string& projectPath)const
	{
		std::vector<std::string> list = Tokenize(m_listValues, ";\r\n|");
	...
	void CModelInputParameterDef::write_xml(pugi::xml_node node)
	{
		node.append_child(GetMemberName(NAME)).set_value(m_name.c_str());
		node.append_child(GetMemberName(CAPTION)).set_value(m_caption.c_str());
		node.append_child(GetMemberName(DESCRIPTION)).set_value(m_description.c_str());
		node.append_child(GetMemberName(TYPE)).set_value(to_string(m_type).c_str());
		node.append_child(GetMemberName(RECT)).set_value(to_string(m_rect).c_str());
		node.append_child(GetMemberName(SEPARATOR_POS)).set_value(to_string(m_beginSecondField).c_str());
		node.append_child(GetMemberName(DEFAULT_VALUE)).set_value(to_string(m_default).c_str());
		node.append_child(GetMemberName(MIN_VALUE)).set_value(to_string(m_min).c_str());
		node.append_child(GetMemberName(MAX_VALUE)).set_value(to_string(m_max).c_str());
		node.append_child(GetMemberName(TEXT_VALUE)).set_value(m_listValues.c_str());
	}


	void CModelInputParameterDef::read_xml(const pugi::xml_node node)
	{
		m_name = node.child(GetMemberName(NAME)).text().as_string();
		m_caption = node.child(GetMemberName(CAPTION)).text().as_string();
		m_description = node.child(GetMemberName(DESCRIPTION)).text().as_string();
		m_type = node.child(GetMemberName(TYPE)).text().as_int();
		m_rect = to_rect(node.child(GetMemberName(RECT)).text().as_string());
		m_beginSecondField = node.child(GetMemberName(SEPARATOR_POS)).text().as_int();
		m_default = node.child(GetMemberName(DEFAULT_VALUE)).text().as_string();
		m_min = node.child(GetMemberName(MIN_VALUE)).text().as_string();
		m_max = node.child(GetMemberName(MAX_VALUE)).text().as_string();
		m_listValues = node.child(GetMemberName(TEXT_VALUE)).text().as_string();
