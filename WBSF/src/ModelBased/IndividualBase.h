...
public:
    ...
    CIndividual(const CIndividual& in)
    {
        operator=(in);
    }
    ...
    virtual bool NeedOverheating()const
    {
        return false;
    }
    virtual void FixAI(double delta)
    {
        m_age = std::max(0.0, m_age + delta);
    }
    ...
    {
        return (IsAlive() || m_death == OLD_AGE) ? (std::min(GetStage(), GetNbStages() - (includeLast ? 0 : 1))) : CBioSIMModelBase::VMISS;    //Report Instar. Return age by default;
    }
    virtual bool IsInDiapause(CTRef TRef)const
    {
        return false;
    }
    ...
    {
        return (size_t)m_age;    //Reports individual's stage
    }
    size_t GetStatus()const
    {
        return m_status;    //Reports individual's status
    }
    ...
    {
        return m_death;    //Reason of death (NOT_DEATH for HEALTY individual)
    }
    void SetDeath(size_t death)
    {
        m_death = death;
    }
    double GetAge()const
    {
        return m_age;    //Reports individual's age
    }
    double GetStageAge()const
    {
        return m_age - GetStage();    //Report age into the current stage [0:1]
    }


    size_t GetLastStatus()const
    {
        return m_lastStatus;    //Reports individual's stage at day¯¹
    }
    size_t GetLastStage()const
    {
        return(size_t)m_lastAge;
    }
    double GetLastAge()const
    {
        return m_lastAge;    //Reports individual's age at day¯¹
    }

    bool HasChangedStage()const
    {
        return GetStage() != GetLastStage();
    }
    bool IsChangingStage(double r)const
    {
        return GetStage() != (size_t)(m_age+r);
    }
    bool HasChangedStatus()const
    {
        return m_status != m_lastStatus;
    }
    bool IsAlive()const
    {
        return m_status == HEALTHY;    //Reports individual not dead (1) or dead (0)
    }
    bool IsCreated(CTRef ref)const
    {
        return ref >= m_creationDate;    //
    }
    ...
    void SetSex(size_t in)
    {
        m_sex = in;
    }

    CTRef GetCreationDate()const
    {
    ...
    const CHost* GetHost()const
    {
        return m_pHost;
    }
    void SetHost(CHost* pHost)
    {
        m_pHost = pHost;
    }

    double  GetBroods()const
    {
        return m_broods;
    }
    double  GetTotalBroods()const
    {
        return m_totalBroods;
    }
    ...
    const CTimeStep& GetTimeStep()const
    {
        assert(GetModel());
        return GetModel()->GetTimeStep();
    }
    const CRandomGenerator& RandomGenerator()const
    {
        assert(GetModel());
        return GetModel()->RandomGenerator();
    }
...
public:
    ...
    const CStand* GetStand()const
    {
        return m_pStand;
    }
    void SetStand(CStand* pStand)
    {
        m_pStand = pStand;
    }
...
public:
    ...
    virtual CHostPtr GetFirstHost()
    {
        return GetNearestHost(NULL);
    }
    virtual const CHostPtr GetFirstHost()const
    {
        return const_cast<CStand*>(this)->GetNearestHost(NULL);
    }

    CBioSIMModelBase* GetModel()
    {
        assert(m_pModel);
        return m_pModel;
    }
    const CBioSIMModelBase* GetModel()const
    {
        assert(m_pModel);
        return m_pModel;
    }
    ...
    {
        assert(m_pModel);
        return m_pModel->GetTimeStep();
    }
    const CRandomGenerator& RandomGenerator()const
    {
        assert(m_pModel);
        return m_pModel->RandomGenerator();
    }
...
inline CBioSIMModelBase* CIndividual::GetModel()
{
    assert(m_pHost);
    return m_pHost->GetModel();
}
inline const CBioSIMModelBase* CIndividual::GetModel()const
{
    assert(m_pHost);
    return m_pHost->GetModel();
}
inline CStand* CIndividual::GetStand()
{
    assert(m_pHost);
    return m_pHost->GetStand();
}

inline CHostPtr CHost::GetNearestHost()
{
    assert(m_pStand);
    return m_pStand->GetNearestHost(this);
}
inline CBioSIMModelBase* CHost::GetModel()
{
    assert(m_pStand);
    return m_pStand->GetModel();
}
inline const CBioSIMModelBase* CHost::GetModel()const
{
    assert(m_pStand);
    return m_pStand->GetModel();
}
