                ...
                type != CModelInputParameterDef::kMVStaticText)
        {
            ...
            case CModelInputParameterDef::kMVListByPos:
            {
                if (!def[i].IsExtendedList())
                {
                    ...
                    std::vector<std::string> listName = def[i].GetList();
            ...
            case CModelInputParameterDef::kMVStaticText:
                break;
            default:
                assert(false);//pas de vérification
...
//        msg = zen::LoadXML(filePath, GetXMLFlag(), "1", *this);
        if (msg)
        {
            m_name = WBSF::GetFileTitle(filePath);
...
CModelInputVector CModelInputVector::operator*(const CModelInputVector& in)const
{
    CModelInputVector out(*this);
    return out *= in;
}
