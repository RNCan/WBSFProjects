...
public:
    ...
    static TSurvivalEquation eq(size_t e)
    {
        assert(e < NB_EQUATIONS);
        return (TSurvivalEquation)e;
    }
    static double GetSurvival(TSurvivalEquation model, const std::vector<double>& P, double T);
    static const char* GetEquationName(TSurvivalEquation model)
    {
        assert(model < NB_EQUATIONS);
        return EQUATION[model][EQ_NAME];
    }
    static const char* GetEquationR(TSurvivalEquation model)
    {
        assert(model < NB_EQUATIONS);
        return EQUATION[model][EQ_R];
    }
    static const char* GetMathPlot(TSurvivalEquation model)
    {
        assert(model < NB_EQUATIONS);
...
public:
    ...
    {
        return std::bitset<CSurvivalEquation::NB_EQUATIONS>::to_string();
    }
    void SetSelection(const std::string& in)
    {
        std::bitset<CSurvivalEquation::NB_EQUATIONS>::operator = (std::bitset<CSurvivalEquation::NB_EQUATIONS>(in));
    }
