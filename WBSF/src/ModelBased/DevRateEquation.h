...
public:
    ...
    enum TDevRateEquation
    {
        ...
        Allahyari_2005 = FIRST_EQUATIONS, Analytis_1977, Angilletta_2006, Bieri_1983, Boatman_2017, Briere1_1999, Briere2_1999, /*damos_2008, */Damos_2011,
        ...
        Lamb_1992, LobryRossoFlandrois_1993, Logan6_1976, Logan10_1976, LoganTb_1979,
        ONeill_1972, Poly1, Poly2, Poly3, /*Poly4,*/
        Ratkowsky_1983, Regniere_1982, Regniere_1982b, Regniere_1987, Regniere_1987b, Regniere_2012, Room_1986, SaintAmant_2021,
        SaintAmant_2022, Schoolfield_1981, SharpeDeMichele_1977, Shi_2011, Shi_2016, Stinner_1974,
        Taylor_1981, Wagner_1988, WangLanDing_1982, WangEngel_1998, YanHunt_1999, Yin_1995/*, YurkPowell_2010*/,
        NB_EQUATIONS
    };



    static TDevRateEquation eq(size_t e)
    {
        assert(e < NB_EQUATIONS);
        return (TDevRateEquation)e;
    }
    ...
    static const char* GetEquationName(TDevRateEquation model)
    {
        assert(model < NB_EQUATIONS);
        return EQUATION[model][EQ_NAME];
    }
    static const char* GetEquationR(TDevRateEquation model)
    {
        assert(model < NB_EQUATIONS);
        return EQUATION[model][EQ_R];
    }
    static const char* GetMathPlot(TDevRateEquation model)
    {
        assert(model < NB_EQUATIONS);
        return EQUATION[model][R_MATH_PLOT];
    }
...
public:
    ...
    {
        return std::bitset<CDevRateEquation::NB_EQUATIONS>::to_string();
    }
    void SetSelection(const std::string& in)
    {
        std::bitset<CDevRateEquation::NB_EQUATIONS>::operator = (std::bitset<CDevRateEquation::NB_EQUATIONS>(in));
    }
