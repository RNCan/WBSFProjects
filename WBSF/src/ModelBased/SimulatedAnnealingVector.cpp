...
ERMsg CSimulatedAnnealingVector::ReadStream(istream& stream)
{
    ...
    {
        msg.ajoute("Invalid dataset size (" + to_string(locSize) + ")");
        return msg;
    }
    resize(locSize);


    std::vector<std::string> locName(size());
    ...
    if (fieldPos < loop.Header().size())
    {
        for (; loop != CSVIterator() && msg; loop++)
        {
            if (fieldPos < (*loop).size())
            {
                ...
                std::vector<std::string>::iterator it = std::find(locName.begin(), locName.end(), name);
                if (it != locName.end())
                {
