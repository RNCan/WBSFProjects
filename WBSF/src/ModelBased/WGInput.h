...
public:
    ...
    {
        return (m_sourceType == FROM_DISAGGREGATIONS);
    }
    bool IsObservation()const
    {
        return (m_sourceType == FROM_OBSERVATIONS);
    }
    bool UseGribs()const
    {
        return IsObservation() && m_bUseGribs;
    }
    bool IsDaily()const
    {
        return (m_sourceType == FROM_OBSERVATIONS) && m_generationType == GENERATE_DAILY;
    }
    bool IsHourly()const
    {
        return (m_sourceType == FROM_OBSERVATIONS) && m_generationType == GENERATE_HOURLY;
    }
    bool UseDaily()const
    {
        return IsDaily() && m_nbDailyStations>0;
    }
    bool UseHourly()const
    {
        return IsHourly()&& m_nbHourlyStations>0;
    }
    size_t GetNbYears()const
    {
        return m_sourceType == FROM_DISAGGREGATIONS ? m_nbNormalsYears : m_lastYear - m_firstYear + 1;
    }
    int GetFirstYear()const
    {
        return m_sourceType == FROM_DISAGGREGATIONS ? 1 - int(m_nbNormalsYears) : m_firstYear;
    }
    int GetLastYear()const
    {
        return m_sourceType == FROM_DISAGGREGATIONS ? 0 : m_lastYear;
    }
    CTM GetTM()const
    {
        return CTM(m_generationType == GENERATE_DAILY ? CTM::DAILY : CTM::HOURLY);
    }
    CTPeriod GetTPeriod()const;

    size_t XVal()const
    {
        return m_bXValidation ? 1 : 0;
    }
    size_t GetNbNormalsToSearch()const
    {
        return  m_nbNormalsStations + XVal();
    }
    size_t GetNbDailyToSearch()const
    {
        return  m_nbDailyStations + XVal();
    }
    size_t GetNbHourlyToSearch()const
    {
        return  m_nbHourlyStations + XVal();
    }
    size_t GetNbObservationToSearch()const
    {
        return  IsHourly() ? GetNbHourlyToSearch() : GetNbDailyToSearch();
    }
