...
std::istream& CParameter::operator << (std::istream& io)
{
    ...
    if (io)
    {
        string tmp;
        std::getline(io, tmp);
        std::vector<std::string> str = WBSF::Tokenize(tmp.c_str(), "|");
...
{
    CTRef date;
    std::vector<std::string> str= WBSF::Tokenize(m_value, "/\\ ");
    ...
    if (str.size() == 2 || str.size() == 3)
    {
        ...
        if (m >= 0 && m < 12 &&
                d >= 0 && d < GetNbDaysPerMonth(year, m))
        {
            date = CTRef(year, m, d);
        }
    }
...
{
    return m_month < 12 && m_day < GetNbDaysPerMonth(m_month) && (m_hour==NOT_INIT||m_hour<24);
}
}
