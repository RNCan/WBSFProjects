...
public:
    ...
    double GoodTrunck(double x)const
    {
        return (m_type == CModelInputParameterDef::kMVReal) ? x : (double)((int)x);
    }
...
public:
    ...
    void SetNbVariation(size_t nbVar)
    {
        m_nbVariation = nbVar;
    }
    ...
    {
        return ERMsg();
//        return zen::LoadXML(filePath, "ParametersVariations", "1", *this);
    }
    ERMsg Save(const std::string& filePath)
    {
        return ERMsg();
        //return zen::SaveXML(filePath, "ParametersVariations", "1", *this);
    }
