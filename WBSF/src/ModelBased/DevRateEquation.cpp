...
bool CDevRateEquation::IsParamValid(CDevRateEquation::TDevRateEquation model, const std::vector<double>& P)
{
    ...
    case Hansen_2011:
        bValid = P[P2] < P[P3];
        break;
    case HilbertLogan_1983:
        bValid = P[P2] < P[P3];
        break;
    case HueyStevenson_1979:
        bValid = P[P2] < P[P3];
        break;
    case Johnson_1974:
        bValid = (P[P2] / P[P1]) > 1;
        break;
    case Kontodimas_2004:
        bValid = P[P1] < P[P2];
        break;
    case LobryRossoFlandrois_1993:
        bValid = (P[P1] < P[P2]) && (P[P2] < P[P3]);
        break;
    case ONeill_1972:
        bValid = P[P2] < P[P3];
        break;
    ...
    case Regniere_2012:
        bValid = P[P2] < P[P3];
        break;
    case SaintAmant_2021:
        bValid = (P[P3] < P[P4]);
        break;
    case SaintAmant_2022:
        bValid = (P[P5] < P[P6]);
        break;
    case Shi_2011:
        bValid = P[P3] < P[P4];
    ...
    case Shi_2016:
        bValid = (P[P1] < P[P2]) && (P[P2] < P[P3]);
        break;
    case Schoolfield_1981:
        bValid = (P[P3] < P[P5]);
        break;
    case SharpeDeMichele_1977:
        bValid = (P[P4] < P[P1]&& P[P1] < P[P6]);
        break;
    case WangLanDing_1982:
        bValid = (P[P2] < P[P3]) && (P[P3] < P[P4]);
        break;
    case WangEngel_1998:
        bValid = (P[P1] < P[P2]) && (P[P2] < P[P3]);
        break;
...
ERMsg CDevRateEquation::GetParamfromString(std::string eq_name, std::string str_param, std::vector<double>& P)
{
    ...
    if (eq != Unknown)
    {
        std::vector<std::string> tmp = WBSF::Tokenize(str_param, " ");
        std::map<string, double, comp> map_P;
        for (size_t i = 0; i < tmp.size(); i++)
        {
            std::vector<std::string> tmp2 = WBSF::Tokenize(tmp[i], "=");
            if (tmp2.size() == 2)
...
CSAParameterVector CDevRateEquation::GetParameters(TDevRateEquation model)
{
    ...
    std::vector<std::string> tmp = WBSF::Tokenize(CDevRateEquation::EQUATION[model][EQ_PARAM], "=[,]|");
    assert(tmp.size() % 4 == 0);
