...
double CHost::GetAI(bool bIncludeLast, size_t generation, size_t sex )const
{
    ...
    if (!empty())
    {
        ...
        AI = (weight.is_init() && weight[SUM] > 0) ? sum[SUM] / weight[SUM] : CBioSIMModelBase::VMISS;
    }
...
void CStand::Live(const CWeatherDay& weather)
{
    assert(GetFirstHost() != NULL);
    GetFirstHost()->Live(weather);
}
void CStand::GetStat(CTRef d, CModelStat& stat, size_t generation)
{
    GetFirstHost()->GetStat(d, stat, generation);
}
bool CStand::AdjustPopulation()
{
    return GetFirstHost()->AdjustPopulation();
}
size_t CStand::GetNbObjectAlive()const
{
    return GetFirstHost()->GetNbObjectAlive();
}
void CStand::HappyNewYear()
{
    GetFirstHost()->HappyNewYear();
}
double CStand::GetAI(bool bIncludeLast)const
{
    return GetFirstHost()->GetAI(bIncludeLast);
}
...
ERMsg CAgeFrequencyVector::Load(std::string& filePath)
{
    ...
    if (msg)
    {
        ...
        {
            std::vector<std::string> elem = WBSF::Tokenize(line, " \t,;");
            assert(elem.size() == 5);
