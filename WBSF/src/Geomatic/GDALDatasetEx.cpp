...
void CGDALDatasetEx::UpdateOption(CBaseOptions& options)const
{
    ...
    if (!((CGeoRect&)options.m_extents).is_init())
    {
...
//			{
//				callback.AddMessage("NbCells = " + to_string(info[NB_VALUE]));
//
//				if (bExtrem)
//					callback.AddMessage("NbExtremPoint = " + to_string(info.GetNbExtremPoint()));
//				callback.AddMessage("NbBox = " + to_string(info.GetNbBox()));
//
//				Pbase = info.GetPBase();
//			}
//		}
...
//			}
//
//			callback.AddMessage("NbPoint = " + to_string(locations.size() + locArrayTmp.size()));
//			callback.PopTask();
//
//
//		}//while
...
CGeoSize CGDALDatasetEx::ComputeBlockSize(double memoryLimit, const CGeoExtents& extents)const
{
    ...
    //	bool bTemporal = !m_scenesPeriod.empty() && period.is_init();
    ...
    for (size_t i = 0; i < GetRasterCount(); i++)
    {
        if (extents.IsRectIntersect(m_internalExtents[i]))
        {
            ...
            //	if (!bTemporal || (m_scenesPeriod[sceneNo].is_init() && period.IsIntersect(m_scenesPeriod[sceneNo])))
            {
...
int CGDALDatasetEx::GetMaxBlockSizeZ(CGeoExtents extents)const
{
    ...
    //
    //		int nbRasterMax = nbRasterAll.is_init() ? (int)nbRasterAll[HIGHEST] : 0;
