...
ERMsg CGribsMap::save(const std::string& file_path)const
{
    ...
    if (msg)
    {
        ...
        {
            string format = it->first.m_TM.Type() == CTM::HOURLY ? "%Y-%m-%d-%H" : "%Y-%m-%d";
...
ERMsg CIncementalDB::save(const std::string& file_path)const
{
    ...
    if (msg)
    {
        string format = m_period.TM().IsHourly() ? "%Y-%m-%d-%H" : "%Y-%m-%d";
...
ERMsg CIncementalDB::GetInvalidTRef(const CGribsMap& gribs, const CTPeriod& period, std::set<CTRef>& invalid)const
{
    ...
    {
        if (!period.is_init() || period.is_inside(it->first))
        {
            const_iterator it_find = find(it->first);
            if (it_find != end())
            {
                const CFileInfo& info_old = it_find->second;


                CFileInfo info_new;
                msg = GetFileInfo(it->second, info_new);
...
{
    assert(gribs.GetEntireTPeriod().TM() == m_period.TM());
    ...
    for (auto it = gribs.begin(); it != gribs.end() && msg; it++)
    {
        ...
        if (m_period.is_inside(it->first))
        {
            CFileInfo info_new;
            msg += GetFileInfo(it->second, info_new);
            if (msg)
...
float CSfcDatasetCached::get_variable(const CGeoPointIndex& in_xy, size_t v)const
{
    ...
    size_t b = get_band(v);
    if (!m_extents.is_inside(in_xy) || b == UNKNOWN_POS)
    ...
    CGeoPointIndex xy = in_xy - m_extents.GetBlockRect(block_ij).UpperLeft();
    assert(m_extents.GetBlockExtents(block_ij).GetPosRect().is_inside(xy));
        ...
        if (bOpenInv)
        {
            ...
            if (m_bVRT)
            {
                ...
                for (size_t i = 0; i < m_internalName.size(); i++)
                {
                    ...
                    vector<string> tmp = Tokenize(title, "_");
                    assert(tmp.size() == 9);
            ...
            else
            {
                ...
                for (size_t i = 0; i < meta_data.size(); i++)
                {
                    ...
                    vector<string> description = Tokenize(meta_data[i]["description"], " =[]\"");
                    if (description.empty())
                    {
                        vector<string> tmp = Tokenize(strName, "-");
                        if (tmp.size() == 2)
                        {
                    ...
                    if (description.size() > 3 && (description[2] == "ISBL" || description[2] == "SFC" || description[2] == "HTGL" || description[2] == "HYBL"))
                    {
                        ...
                        if (var < NB_VAR_GRIBS && m_variables_to_load.test(var) && !description.empty())
                        {
                            ...
                            {
                                size_t level = std::stoi(description[0]);
...
void CSfcDatasetCached::get_weather(const CGeoPoint& pt, CWeatherDay& data)const
{
    ...
    if (nearest.m_x >= 0 && nearest.m_y >= 0)
    {
        ...
        for (size_t v = 0; v < NB_VAR_H; v++)
        {
            if (!data[TVarH(v)].is_init())//keep the value of the first grib
            {
                if (sumP[v].is_init())
                {
...
ERMsg CSfcGribDatabase::CreateDatabase(const std::string& filePath)
{
    ...
    msg = pDB->Open(filePath, CWeatherDatabase::modeWrite);
    if (msg)
    {
        pDB->Close();
...
int CSfcGribDatabase::GetVersion(const std::string& filePath)
{
    return 1;
}
...
ERMsg CSfcGribDatabase::Open(const std::string& filePath, CWeatherDatabase::TOpenMode flag, CCallback& callback, bool bSkipVerify)
{
    ERMsg msg;
...
ERMsg CSfcGribDatabase::Update(const CGribsMap& gribs, const CLocationVector& locationsIn, CCallback& callback)
{
    ...
    if (Gribs_period.TM() != m_period.TM())
    {
        msg.ajoute("Gribs mismatch Hourly/Daily");
        return msg;
    }

    if (!Gribs_period.is_intersect(m_period))
    {
    ...
    m_bIsHourly = Gribs_period.TM() == CTM::HOURLY;
    ...
    if (bIncremental)
    {
        if (!m_pDB->empty())
        {
            ...
            if (bIncremental == false)
            {
                ...
                msg += m_pDB->Open(file_path, CWeatherDatabase::modeEdit, callback, true);
...
ERMsg CSfcGribDatabase::ExtractStation(CTRef TRef, const std::string& file_path, CWeatherStationVector& stations, CCallback& callback)
{
    ...
    if (msg)
    {
        ...
        //			if (data[H_TMIN].is_init() && data[H_TMAX].is_init() && !data[H_TAIR].is_init() && m_variables.test(H_TAIR))
