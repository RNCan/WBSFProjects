...
ERMsg CLandsatDataset::OpenInputImage(const std::string& filePath, const CBaseOptions& options)
{
    ...
    if (msg)
    {
        ...
        {
            msg.ajoute("ERROR: input image bands count (" + to_string(int(GetRasterCount())) + ") must be a multiple of LANDSAT scene size (" + to_string(int(options.GetSceneSize())) + ")");
        }
...
bool CLandsatPixel::is_init()const
{
...
bool CLandsatPixel::is_init(TIndices i)const
{
    LandsatDataType noData = GetLandsatNoData();
...
Color8 CLandsatPixel::R(CBaseOptions::TRGBTye type, const CBandStats& stats)const
{
    ...
    case CBaseOptions::OLD_NATURAL:
        pix_val = Color8(max(0.0, min(254.0, (((double)at(B3) - 2 * 90.0) / (2 * (1000.0 - 90.0))) * 254.0)));
        break;
    //case CBaseOptions::LANDWATER:
    //  pix_val = Color8(max(0.0, min(254.0, (((double)at(B4) + 150.0) / 6150.0) * 254.0)));
    //        break;
    case CBaseOptions::LANDWATER:
        pix_val = Color8(max(0.0, min(1.0, ((double)at(B4) - stats[B4].m_min) / (stats[B4].m_max - stats[B4].m_min))) * 254);
        break;
    case CBaseOptions::NATURAL:
        pix_val = Color8(pow(max(0.0, min(1.0, ((double)at(B4) - (stats[B4].m_mean - KK * stats[B4].m_sd)) / (2 * KK * stats[B4].m_sd))), 1) * 254.0);
        break;
    case CBaseOptions::TRUE_COLOR:
        pix_val = Color8(max(0.0, min(1.0, ((double)at(B1) - 7640) / ((double)8697.0 - 7640.0))) * 254.0);
        break;
...
Color8 CLandsatPixel::G(CBaseOptions::TRGBTye type, const CBandStats& stats)const
{
    ...
    case CBaseOptions::OLD_NATURAL:
        pix_val = Color8(max(0.0, min(254.0, (((double)at(B2) - 2 * 170.0) / (2 * (1050.0 - 170.0))) * 254.0)));
        break;
    //case CBaseOptions::LANDWATER:
    //  pix_val = Color8(max(0.0, min(254.0, (((double)at(B5) + 190.0) / 5190.0) * 254.0)));
    //        break;
    case CBaseOptions::LANDWATER:
        pix_val = Color8(max(0.0, min(1.0, ((double)at(B5) - stats[B5].m_min) / (stats[B5].m_max - stats[B5].m_min))) * 254);
        break;
    case CBaseOptions::NATURAL:
        pix_val = Color8(pow(max(0.0, min(1.0, ((double)at(B5) - (stats[B5].m_mean - KK * stats[B5].m_sd)) / (2 * KK * stats[B5].m_sd))), 1) * 254.0);
        break;
    case CBaseOptions::TRUE_COLOR:
        pix_val = Color8(max(0.0, min(1.0, ((double)at(B2) - 7640) / ((double)9570.0 - 7640.0))) * 254.0);
        break;
...
Color8 CLandsatPixel::B(CBaseOptions::TRGBTye type, const CBandStats& stats)const
{
    ...
    case CBaseOptions::NO_RGB:
        break;
    case CBaseOptions::OLD_NATURAL:
        pix_val = Color8(max(0.0, min(254.0, (((double)at(B1) - 2 * 130.0) / (2 * (780.0 - 130.0))) * 254.0)));
        break;
    //case CBaseOptions::LANDWATER:
    //  pix_val = Color8(max(0.0, min(254.0, (((double)at(B3) + 200.0) / 2700.0) * 254.0)));
    //break;
    case CBaseOptions::LANDWATER:
        pix_val = Color8(max(0.0, min(1.0, ((double)at(B3) - stats[B3].m_min) / (stats[B3].m_max - stats[B3].m_min))) * 254);
        break;
    case CBaseOptions::NATURAL:
        pix_val = Color8(pow(max(0.0, min(1.0, ((double)at(B3) - (stats[B3].m_mean - KK * stats[B3].m_sd)) / (2 * KK * stats[B3].m_sd))), 1) * 254.0);
        break;
    case CBaseOptions::TRUE_COLOR:
        pix_val = Color8(max(0.0, min(1.0, ((double)at(B3) - 7510.0) / ((double)9300.0 - 7510.0))) * 254.0);
        break;
