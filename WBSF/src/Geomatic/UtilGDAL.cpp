...
ERMsg OpenInputImage(const string& filePath, GDALDataset** poInputDS, double srcNodata, bool bUseDefaultNoData, bool bReadOnly)
{
    ...
    *poInputDS = (GDALDataset*)GDALOpenEx(filePath.c_str(), flag, NULL, NULL, NULL);
...
double GetTypeLimit(short eType, bool bLow)
{
    ...
    case GDT_Unknown:
        ...
        break;
    case GDT_Byte:
        limit = bLow ? 0 : UCHAR_MAX;
        break;
    case GDT_UInt16:
        limit = bLow ? 0 : USHRT_MAX;
        break;
    case GDT_Int16:
        limit = bLow ? SHRT_MIN : SHRT_MAX;
        break;
    case GDT_UInt32:
        limit = bLow ? 0 : UINT_MAX;
        break;
    case GDT_Int32:
        limit = bLow ? INT_MIN : INT_MAX;
        break;
    case GDT_Float32:
        limit = bLow ? -FLT_MAX : FLT_MAX;
        break;
    case GDT_Float64:
        limit = bLow ? -DBL_MAX : DBL_MAX;
        break;
    case GDT_CInt16:
        limit = bLow ? SHRT_MIN : SHRT_MAX;
        break;
...
{
    if (v > -FLT_MAX && v < FLT_MAX && v != MISSING_DATA && !isnan(v) && isfinite(v))
    {
...
//open output image
ERMsg OpenOutputImage(const string& filePath, GDALDataset* poInputDS, GDALDataset** poOutputDS, const char* outDriverName, short cellType, int nbBand, double dstNodata, const std::vector<std::string>& createOptions, bool bOverwrite, const CGeoExtents& extentsIn, bool bUseDefaultNoData)
{
...
CGeoExtents GetExtents(GDALDataset* poDataset)
{
    ...
    if (poDataset)
    {
        ...
        extents.SetPrjID(SRS.IsGeographic() ? PRJ_GEOGRAPHIC : PRJ_PROJECTED);
...
int CBaseOptions::GetOptionIndex(const char* name)
{
    ...
    {
        //if( strcasecmp(name, OPTIONS_DEF[i].m_name)==0 )
        if (IsEqual(name, OPTIONS_DEF[i].m_name))
            index = i;
    }
...
{
    std::vector<std::string> tmp = TokenizeQuoted(str, " \t");
    ...
    for (int i = 0; i < argc; i++)
    {
        ...
        strncpy(pStr, tmp[i].c_str(), tmp[i].size() + 1);
        args[i] = pStr;
    }
...
ERMsg CBaseOptions::ParseOptions(int argc, char* argv[])
{
    boost::scoped_array<char*> args(new char* [argc]());

    for (int i = 0; i < argc; i++)
    {
        string str = argv[i];//WBSF::UTF8(
        char* pStr = new char[str.length() + 1]();
        strncpy(pStr, str.c_str(), str.size() + 1);
        args[i] = pStr;
    }
...
ERMsg CBaseOptions::ParseOption(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        {
            //option argument
            COptionDefMap::const_iterator it = std::find_if(m_optionsDef.begin(), m_optionsDef.end(), [optionName](COptionDefMap::const_reference od)
            {
                return IsEqual(od.second.m_name, optionName);//strcasecmp(od.second.m_name, optionName) == 0;
            });
            if (it != m_optionsDef.end())
            {
                ...
                {
                    string error = FormatA("ERROR: option %s don't have the good number of parameters, use -? for more help.\n", argv[i]);
                    msg.ajoute(error);
                }
            }
            else
...
ERMsg CBaseOptions::ProcessOption(int& i, int argc, char* argv[])
{
    ...
    {
        m_bExtents = true;
        m_extents.m_xMin = CPLAtofM(argv[i + 1]);
        i++;
        m_extents.m_yMin = CPLAtofM(argv[i + 1]);
        i++;
        m_extents.m_xMax = CPLAtofM(argv[i + 1]);
        i++;
        m_extents.m_yMax = CPLAtofM(argv[i + 1]);
        i++;
    ...
    {
        //m_bSize=true;
        m_extents.m_xSize = atoi(argv[i + 1]);
        i++;
        m_extents.m_ySize = atoi(argv[i + 1]);
        i++;
    }
    else if (IsEqual(argv[i], "-tr"))
    {
        m_bRes = true;
        m_xRes = CPLAtofM(argv[i + 1]);
        i++;
        m_yRes = CPLAtofM(argv[i + 1]);
        i++;
    }
    ...
    {
        m_CPU = min(2 * omp_get_num_procs(), atoi(argv[i + 1]));
        i++;
        if (m_CPU <= 0)
    ...
    {
        string tmp = argv[++i];
        std::vector<std::string> levels = Tokenize(tmp, "{,}");
...
void CBaseOptions::PrintTime()
{
    ...
    {
        cout << "\n";
        cout << "Time to read images = " << SecondToDHMS(m_timerRead.elapsed().wall/1e9) << endl;
        cout << "Time to process = " << SecondToDHMS(m_timerProcess.elapsed().wall/1e9) << endl;
        cout << "Time to write image = " << SecondToDHMS(m_timerWrite.elapsed().wall/1e9) << endl;
    }
}
//
