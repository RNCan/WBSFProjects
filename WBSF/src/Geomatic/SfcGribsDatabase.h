...
#include "WeatherBased/HourlyDatabase.h"
...
//*****************************************************************************************************************
class CIncementalDB : public std::map<CTRef, CFileInfo>
{
public:

    void clear()
    {
        std::map<CTRef, CFileInfo>::clear();
...
public:
    ...
    bool is_cached(size_t i, size_t j)const
    {
        assert(is_block_inside(i, j));
        return block(i,j) != nullptr;
    }
    size_t get_band(size_t v)const
    {
        return m_bands[v];
    }
    const GribVariables& get_variables()const
    {
        return m_variables;
    }
...
protected:
    ...
    bool is_cache_init()const
    {
        return !m_blocks.empty();
    }
    void load_block(size_t i, size_t j);
    bool is_block_inside(size_t i, size_t j)const
    {
        return	j < m_blocks.size() && i < m_blocks[j].size();
    }
    CSfcWeatherBlockPtr& block(size_t i, size_t j)
    {
        return	m_blocks[j][i];
    }
    const CSfcWeatherBlockPtr& block(size_t i, size_t j)const
    {
        return	m_blocks[j][i];
    }
...
public:
    ...
    ERMsg Open(const std::string& filePath, CWeatherDatabase::TOpenMode flag = CWeatherDatabase::modeRead, CCallback& callback= DEFAULT_CALLBACK, bool bSkipVerify=false);
    ...
    const CDHDatabaseBase* GetDB()const
    {
        return m_pDB.get();
    }
    std::shared_ptr < CDHDatabaseBase>& GetDBPrt()
    {
        return m_pDB;
    }
