...
public:
    ...
    virtual bool SometingInRect(const CGeoRect& rect)const;
    double Pente(const CGeoPoint& p)const
    {
        return (p.m_y - m_y) / (p.m_x - m_x);
    }
...
public:
    ...
    {
        return m_header.GetBoundingBox();
    }
    void SetBoundingBox(const CGeoRect& bounds)
    {
        m_header.SetBoundingBox(bounds);
    }
    ...
    CShapeFileHeader::TShape GetShapeType()const
    {
    ...
    const CDBF3& GetDBF()const
    {
        return m_infoDBF;
    }
    void SetDBF(const CDBF3& newDBF)
    {
        m_infoDBF = newDBF;
    }
    int GetCount()const
    ...
    CSFRecordArray& GetRecords()
    {
        return m_records;
    }
    const CSFRecordArray& GetRecords()const
    {
        return m_records;
    }
