...
public:
    ...
    const CCallbackTask& back()const
    {
        return top();
    }
};
...
public:
    ...
    void PushTask(const std::string& description, size_t nbStep, size_t stepBy = 1)
    {
        PushTask(description, (double)nbStep, (double)stepBy);
    }
    void PopTask();
    ...
    void SetUserCancel(bool bCancel = true)
    {
        bCancel ? m_cancelEvent.set() : m_cancelEvent.unset();
    }
    void SetPause(bool bPause)
    {
        bPause ? m_pauseEvent.set() : m_pauseEvent.unset();
    }
    void WaitPause();
    ...
    void SetPumpMessage(bool in)
    {
        m_bPumpMessage=in;
    }
    void SetUserCancelMessage(const std::string& msg)
    {
        m_userCancelMsg=msg;
    }
    ...
    std::string GetDlgMessageText()const
    {
        return m_messageDlgAccumulator;
    }
    std::string GetCurrentTaskMessageText()const
    {
    ...
    const CCallbackTaskStack& GetTasks()const
    {
        return m_tasks;
    }
