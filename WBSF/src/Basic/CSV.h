...
public:
    ...
    //
    CSVIterator(std::istream& str, const char* pD = ",", bool bHeader = true, bool bDQ = false) :m_str(str.good() ? &str : NULL), m_row(pD, bDQ), m_header(pD, true)
    {
    ...
    CSVIterator& operator++()
    {
        ...
        {
            (*m_str) >> m_row;
            m_str = (!m_row.empty() || m_str->good()) ? m_str : NULL;
        }
        return *this;
    }
    // Post increment
