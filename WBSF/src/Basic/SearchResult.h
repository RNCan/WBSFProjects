...
public:
    ...
    bool operator ==(const CSearchResult& in)const
    {
        return m_index == in.m_index;
    }
    bool operator !=(const CSearchResult& in)const
    {
        return !operator==(in);
    ...
    bool operator > (const CSearchResult& in)const
    {
        return m_virtual_distance > in.m_virtual_distance;
    }
    bool operator < (const CSearchResult& in)const
    {
        return m_virtual_distance < in.m_virtual_distance;
...
public:
    ...
    bool operator ()(const CSearchResult& in1, const CSearchResult& in2)const
    {
        ...
        {
        case INDEX:
            bRep = f*in1.m_index < f*in2.m_index;
            break;
        case DISTANCE:
            bRep = f*in1.m_distance < f*in2.m_distance;
            break;
        case DELTA_ELEVATION:
            bRep = f*in1.m_deltaElev < f*in2.m_deltaElev;
            break;
        case ABS_DELTA_ELEVATION:
            bRep = f*fabs(in1.m_deltaElev) < f*fabs(in2.m_deltaElev);
            break;
        case DELTA_SHORE:
            bRep = f*in1.m_deltaShore < f*in2.m_deltaShore;
            break;
        case ABS_DELTA_SHORE:
            bRep = f*fabs(in1.m_deltaShore) < f*fabs(in2.m_deltaShore);
            break;
        case VIRTUAL_DISTANCE:
            bRep = f*fabs(in1.m_virtual_distance) < f*fabs(in2.m_virtual_distance);
            break;
        case WEIGHT:
            bRep = f*in1.GetD() < f*in2.GetD();
            break;
        default:
            assert(false);
        }
