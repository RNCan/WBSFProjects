...
double Linearize(double x2, double x1, double y1, double x3, double y3)
{
    ...
    assert(x1 != x3); // x1 = x3 => not a function
...
double e0(double Tmin, double Tmax)
{
    return (e0(Tmax) + e0(Tmin)) / 2;
}
...
double Hr2Td(double Tair, double Hr)
{
    ...
    assert(Td - 273.15 > -99 && Td - 273.15 < 99);
...
{
    double Psat = e0(Tair) * 1000;//kPa --> Pa
...
{
    double Psat = e0(Tmin, Tmax) * 1000;//kPa --> Pa
    double Hr = 100 * Pv / Psat;
...
double GetErbs(double Tmin[3], double Tmax[3], double t)
{
    ...
    size_t ii = t < 05 ? 0 : 1;
    double Tmin2 = Tmin[i];
    double Tmax2 = Tmax[ii];
    double mean = (Tmin2 + Tmax2) / 2;
    double range = Tmax2 - Tmin2;
...
void ComputeSlopeAndAspect(double latDeg, double exposition, double& slopePourcent, double& aspect)
{
    ...
    slopePourcent = (float)tan(slope) * 100;
    assert(slopePourcent >= 0);//&& fPentePourcent <= 100 );
    assert(nbRun == 500 || fabs(exposition - GetExposition(latDeg, slopePourcent, aspect)) < 0.001);
}
