...
std::vector<std::pair<int, int>> CGeoExtents::GetBlockList(size_t max_cons_row, size_t max_cons_col)
{
    ...
    if (max_cons_row == NOT_INIT || max_cons_row>size_t(YNbBlocks()))
        max_cons_row = YNbBlocks();

    if (max_cons_col == NOT_INIT || max_cons_col>size_t(XNbBlocks()))
        max_cons_col = XNbBlocks();

    int nbYpass = (int)ceil((double)YNbBlocks() / max_cons_row);
    int nbXpass = (int)ceil((double)XNbBlocks() / max_cons_col);

    for (int yPass = 0; yPass<nbYpass; yPass++)
        for (int xPass = 0; xPass<nbXpass; xPass++)
            for (int yBlock = 0; yBlock<int(max_cons_row); yBlock++)
                for (int xBlock = 0; xBlock < int(max_cons_col); xBlock++)
                {
...
typedef std::deque<CDistancePoint> CCDistancePointVector;
static  bool cmp_less_first (CDistancePoint i, CDistancePoint j)
{
...
void CGeoExtents::GetNearestCellPosition(const CGeoPoint& pt, int nbPoint, CGeoPointIndexVector& ptArray)const
{
    ...
    assert( ptArray.size() == size_t(nbPoint));
}
...
ERMsg BuildVRT(string filePath, vector<string> fileList, bool bQuiet, std::string EXEPath )
{
    ERMsg msg;
