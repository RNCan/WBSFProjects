...
//----------------------------------------------------------------------
//	annPkSearch - search for the k nearest neighbors
//----------------------------------------------------------------------

void ANNkd_tree::annPkSearch(
    ANNpoint			q,			// the query point
    int				k,			// number of near neighbors to return
    ANNidxArray			nn_idx,			// nearest neighbor indices (returned)
    ANNdistArray		        dd,			// the approximate nearest neighbor
    double				eps)			// the error bound
{
    ...
    int ANNkdDim = dim;						// copy arguments to static equivs
    ANNpoint ANNkdQ = q;
    ANNpointArray ANNkdPts = pts;
    //ANNptsVisited = 0;					// initialize count of points visited
    ...
    double ANNkdMaxErr = ANN_POW(1.0 + eps);
    //ANN_FLOP(2)							// increment floating op count
    ANNmin_k *ANNkdPointMK = new ANNmin_k(k);		        // create set for closest k points
    // search starting at the root

    root->ann_Psearch(annBoxDistance(q, bnd_box_lo, bnd_box_hi, dim), ANNkdPointMK, ANNkdPts, ANNkdQ, ANNkdDim, ANNkdMaxErr);
...
void ANNkd_split::ann_Psearch(ANNdist box_dist, ANNmin_k *ANNkdPointMK, ANNpointArray ANNkdPts, ANNpoint ANNkdQ, int ANNkdDim, double ANNkdMaxErr)
{
    // check dist calc termination condition
    //if (ANNmaxPtsVisited && ANNptsVisited > ANNmaxPtsVisited) return;
    ...
    if (cut_diff < 0)  					// left of cutting plane
    {
        child[ANN_LO]->ann_Psearch(box_dist, ANNkdPointMK, ANNkdPts, ANNkdQ, ANNkdDim, ANNkdMaxErr);// visit closer child first
        ...
        // visit further child if close enough
        if (box_dist * ANNkdMaxErr < ANNkdPointMK->max_key())
            child[ANN_HI]->ann_Psearch(box_dist, ANNkdPointMK, ANNkdPts, ANNkdQ, ANNkdDim, ANNkdMaxErr);

    }
    else  								// right of cutting plane
    {
        child[ANN_HI]->ann_Psearch(box_dist, ANNkdPointMK, ANNkdPts, ANNkdQ, ANNkdDim, ANNkdMaxErr);// visit closer child first
        ...
        // visit further child if close enough
        if (box_dist * ANNkdMaxErr < ANNkdPointMK->max_key())
            child[ANN_LO]->ann_Psearch(box_dist, ANNkdPointMK, ANNkdPts, ANNkdQ, ANNkdDim, ANNkdMaxErr);
...
void ANNkd_leaf::ann_Psearch(ANNdist box_dist, ANNmin_k *ANNkdPointMK, ANNpointArray ANNkdPts, ANNpoint ANNkdQ, int ANNkdDim, double ANNkdMaxErr)
{
    ANNdist dist;				// distance to data point
    ANNcoord* pp;				// data coordinate pointer
    ANNcoord* qq;				// query coordinate pointer
