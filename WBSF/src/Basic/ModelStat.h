...
public:
    ...
    {
        assert(stat >= 0 && stat < size());
        assert(reference >= 0 && reference < size());
...
public:
    ...
    CModelStatVector(size_t size = 0, CTRef firstDate = CTRef(), short nbStat = 0, double initValue = 0, const std::string& header = "")
    {
        m_missingValue = -9999;
        Init(size, firstDate, nbStat, initValue, header);
    }
    CModelStatVector(const CTPeriod& p, size_t nbStat = 0, double initValue = 0, const std::string& header = "")
    {
        m_missingValue = -9999;
        Init(p, nbStat, initValue, header);
    }
    ...
    void Init(size_t size, CTRef firstDate, size_t nbStat, double initValue = 0, const std::string& header = "")
    {
        m_firstTRef = firstDate;
        m_nbStat = nbStat;
        m_header = header;
        resize(size, initValue);
    }
    void Init(const CTPeriod& p, size_t nbStat, double initValue = 0, const std::string& header = "")
    {
        Init(p.size(), p.begin(), nbStat, initValue, header);
    }
    void Init(const CModelStatVector& in, const CTM& TM=CTM(), size_t stat = MEAN);
    ...
    void resize(size_t size, size_t nbStat, double defaultValue)
    {
        m_nbStat = nbStat;
    ...
    bool HaveData()const;
    size_t GetFirstIndex(size_t stat, const std::string& op, double threshold, int nbDayBefore = 1, const CTPeriod& p = CTPeriod())const;
    size_t GetLastIndex(size_t stat, const std::string& op, double threshold, int nbDayAfter = 1, const CTPeriod& p = CTPeriod())const;
    ...
    {
        return CTPeriod(GetFirstTRef(v, op, threshold, nbDayBefore, p), GetLastTRef(v, op, threshold, nbDayAfter, p));
    }
    ...
    void SetFirstTRef(CTRef firstDate)
    {
        m_firstTRef = firstDate;
    }
    ...
    const CModelStat& at(CTRef d)const
    {
        assert(d.TM() == m_firstTRef.TM());
        return at(d - m_firstTRef);
    }
    CModelStat& at(CTRef d)
    {
        assert(d.TM() == m_firstTRef.TM());
        return at(d - m_firstTRef);
    }
    using CModelStatVectorBase::operator[];
    const CModelStat& operator [](CTRef d)const
    {
        return at(d);
    }
    CModelStat& operator [](CTRef d)
    {
        return at(d);
    }
    ...
    {
        return GetTPeriod().is_inside(d);
    }
    int32_t GetNbDay(size_t v, const std::string& op, const CTPeriod& p, bool bConsecutive, double MISSING = -999)const;

    void WriteStream(std::ostream& stream)const;
    ERMsg ReadStream(std::istream& stream);

    void ConvertValue(float value1, float value2);
    size_t GetNbStat()const
    {
        return m_nbStat;
    }
...
public:
    ...
    void Init(size_t size = 0, CTRef firstDate = CTRef(), double initValue = 0)
    {
        CModelStatVector::Init(size, firstDate, T, initValue, U ? U : "");
    }
    void Init(const CTPeriod& p, double initValue = 0)
    {
        CModelStatVector::Init(p, T, initValue, U ? U : "");
    }
...
public:
    ...
    {
        m_period = period;
        CMatrix<T>::resize(m_period.size(), nbVars);
    }


    CTReferencedMatrix(const CTReferencedMatrix& in)
    {
        operator=(in);
    }
    ...
    CTReferencedMatrixIterator<T> operator[](const CTRef& TRef)const
    {
        return at(TRef);
    }
    CTReferencedMatrixIterator<T> operator[](const CTRef& TRef)
    {
        return at(TRef);
    }
    CTReferencedMatrixIterator<T> at(const CTRef& TRef)const
    {
        return CTReferencedMatrixIterator<T>(*this, TRef - m_period.begin());
    }
    CTReferencedMatrixIterator<T> at(const CTRef& TRef)
    {
        return CTReferencedMatrixIterator<T>(*this, TRef - m_period.begin());
    }

    CTRef GetTRef(size_t i)const
    {
        return (m_period.begin() + i);
    }
    ...
    const T& operator()(const CTRef& TRef, size_t col)const
    {
        CMatrix<T>& me = *this;
        return me[TRef - m_period.begin()][col];
    }
...
struct ModelInterface
{
    ...
    void Transform(const CTM& TM, const CModelStatVector& in, CModelStatVector& out, size_t s)
    {
        Transform(CTTransformation(in.GetTPeriod(), TM), in, out, s);
    }
    void Transform(const CTTransformation& TT, const CModelStatVector& in, CModelStatVector& out, size_t s);
    void Transform(const CTM& TM, const CModelStatVector& in, CTStatMatrix& out)
    {
        Transform(CTTransformation(in.GetTPeriod(), TM), in, out);
    }
