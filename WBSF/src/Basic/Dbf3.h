...
public:
    ...
    CDBFFieldIdentifier(const std::string& name = "", short index = -999)
    {
        Set(name, index);
    };
...
public:
    ...
    {
        m_type = (char)toupper(type);
        assert(m_type == 'C' || m_type == 'D' || m_type == 'N' || m_type == 'L' || m_type == 'M');
    }
    ...
    void SetDecimalCount(unsigned char decimelCount)
    {
        m_decimalCount = decimelCount;
    }
    char GetWorkArea()const
    {
        return m_workArea;
    }
    void SetWorkArea(char workArea)
    {
        m_workArea = workArea;
    }
    char GetMDX()const
...
public:
    ...
    int GetNbField()const
    {
        return (int)m_tableField.size();
    }
    const CDBFField& GetField(int index)
    {
        return m_tableField[index];
    }
    void SetElement(int index, bool bElement)
    {
        m_record[index].SetElement(m_tableField[index], bElement);
    }
    void SetElement(int index, int nElement)
    {
        m_record[index].SetElement(m_tableField[index], nElement);
    }
    void SetElement(int index, float fElement)
    {
        m_record[index].SetElement(m_tableField[index], fElement);
    }
    void SetElement(int index, const char* sElement)
    {
    ...
    {
        return m_record[index].GetElement();
    }
...
public:
    ...
    bool EndOK(Archive& io)
    {
        char test(0);
        io >> test;// .Read(&test, 1);
        return test == CDBFField::DBF_FILE_END;
    }
    ...
    void ResetField();
    int AddField(const CDBFField& field);
    ...
    void SetNbRecord(int32_t nbRecord);
    ...
    int32_t GetNbRecord()const
    {
        return (int32_t)m_records.size();
    }
