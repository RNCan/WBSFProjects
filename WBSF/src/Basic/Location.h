...
public:
    ...
    friend std::ostream& operator << (std::ostream& stream, const SiteSpeceficInformationMap& data)
    {
        return data << stream;
    }
    friend std::istream& operator >> (std::istream& stream, SiteSpeceficInformationMap& data)
    {
        return data >> stream;
    }
...
public:
    ...
    static const char* GetMemberName(size_t i)
    {
    ...
    virtual ~CLocation() {}
    virtual ERMsg SaveData(const std::string& filePath, CTM TM = CTM(), char separator = ',')const
    {
        assert(false);
        return ERMsg();
    }
    ...
    void SetDataFileName(std::string in)
    {
        SetDefaultSSI(CLocation::DATA_FILE_NAME, in);
    }
    ...
    bool UseIt()const
    {
        std::string tmp = GetDefaultSSI(CLocation::USED_IT);
        return tmp.empty() || tmp != "0";
    }
    void UseIt(bool in)
    {
        SetDefaultSSI(CLocation::USED_IT, in ? "1" : "0");
    }
    ...
    friend std::ostream& operator<<(std::ostream &s, const CLocation& pt)
    {
        pt >> s;
        return s;
    }
    friend std::istream& operator>>(std::istream &s, CLocation& pt)
    {
        pt << s;
        return s;
    }
...
protected:
    ...
    static std::vector<std::string> DEFAULT_SSI_TITLE;
...
public:
    ...
    bool operator ()(const CLocation& in)const
    {
        return WBSF::IsEqual(in.m_name, m_name, m_bCase);
    }
...
public:
    ...
    bool operator ()(const CLocation& in)const
    {
        return WBSF::IsEqual(in.m_ID,m_ID, m_bCase);
    }
...
public:
    ...
    bool operator ()(const CLocation& in)const
    {
        return WBSF::IsEqual(in.GetDataFileName(), m_dataFileName, m_bCase);
    }
...
public:
    ...
    bool operator ()(const CLocation& in)const
    {
        return WBSF::IsEqual(in.GetSSI(m_SSI), m_value, m_bCase);
    }
...
public:
    ...
    {
        return ExtractNominatimName(*this, bReplaceAll, bName, bState, bCountry, callback);
    }
    ERMsg ExtractOpenTopoDataElevation(bool bReplaceAll, size_t eProduct, size_t eINterpol, CCallback& callback)
    {
        return ExtractOpenTopoDataElevation(*this, bReplaceAll, eProduct, eINterpol, callback);
    }
    ERMsg ExtractShoreDistance(bool bReplaceAll, CCallback& callback)
    {
        return ExtractShoreDistance(*this, bReplaceAll, callback);
    }
    ...
    std::ostream& operator << (std::ostream& stream)const;
    std::istream& operator >> (std::istream& stream);
    friend std::ostream& operator << (std::ostream& stream, const CLocationVector& data)
    {
        return data << stream;
    }
    friend std::istream& operator >> (std::istream& stream, CLocationVector& data)
    {
        return data >> stream;
    }
    ...
    {
        std::vector<std::string> header(CLocation::SITE_SPECIFIC_INFORMATION);
        for (size_t i = 0; i < CLocation::SITE_SPECIFIC_INFORMATION; i++)
            header[i] = CLocation::GetMemberTitle(i);

        std::vector<std::string> SSI = GetSSIHeader();
    ...
    std::vector<std::string> GetSSIHeader()const
    {
        return empty() ? std::vector<std::string>() : front().GetSSIHeader();
    }
    ...
    std::vector<std::string> GetHeaderFromData()const;
...
namespace zen
{
		...
		void writeStruc(const WBSF::CLocation& loc, XmlElement& output)
	{
		...
		WBSF::vector<string> header = loc.GetSSIHeader();
