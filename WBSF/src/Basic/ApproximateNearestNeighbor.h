...
public:
    ...
    friend std::ostream& operator << (std::ostream& s, const CApproximateNearestNeighbor& ANN)
    {
        return (ANN >> s);
    }
    friend std::istream& operator >> (std::istream& s, CApproximateNearestNeighbor& ANN)
    {
        return (ANN << s);
    }
    ...
    bool empty()const
    {
        return m_nSize*m_nbDimension == 0;
    }
    void clear();
    size_t size()const
    {
    ...
    size_t SearchPos2LocPos(size_t i)const
    {
        assert(m_positions.empty() || (i < m_positions.size()));
        return m_positions.empty() ? i : m_positions[i];
    }
