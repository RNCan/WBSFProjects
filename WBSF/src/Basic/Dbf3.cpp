...
const char CDBFField::END_HEAD = 0x0D;
const char CDBFField::DBF_FILE_END = 0x1A;
...
int CDBF3::AddField(const CDBFField& field)
{
    DestroyUniqueIDCtrl();
...
void CDBF3::SetNbRecord(int32_t nbRecord)
{
    DestroyUniqueIDCtrl();
...
ERMsg CDBF3::Write(const string& filePath)const
{
    ...
    if (msg)
    {
        try
        {
            ...
            int32_t nbRecord = (int32_t)m_records.size();
...
{
    memset(m_name, 0, 11);
    memset(m_reserved1, 0, 4);
    memset(m_reserved2, 0, 3);
    memset(m_reserved3, 0, 10);
