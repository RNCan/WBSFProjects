...
public:
    ...
    CVariableBound(double low = -1.0E25, double hi = 1.0E25)
    {
        m_lowerBound = low, m_upperBound = hi;
    }
    ...
    bool IsOutOfBound(double& value)const
    {
...
public:
    ...
    std::string to_string()const
    {
        return m_name + "=" + FormatA("%.7g", m_initialValue) + m_bounds.to_string();
    }
    CSAParameter& from_string(const std::string& in)
    {
        return operator=(FromString(in));
    }


    static CSAParameter FromString(const std::string& in)
    {
