...
namespace WBSF
{
	...
	int old_Sol9(float inlatit, float inelev, float inslope, float inazimuth, float* expin)
	{
		...
		for (month = 0; month < 12; ++month)
		{
			...
			julian_day = (float)(midpnt[month] + 1);//+1, for base 1
	...
	int old_ExposureIndices(float exposure_index[12], float latit, float elev, float psi)
	{
		...
		cos2lat = (float)cos(2*PI * latit / 360.f);
		cos2lat = cos2lat * cos2lat;
		sin2lat = (float)sin(2*PI * latit / 360.f);
	...
	void CWeatherGenerator::CompleteSimpleVariables(CSimulationPoint& simulationPoint, CWVariables variables)
	{
		...
		for (CTRef TRef = period.begin(); TRef <= period.end(); TRef++)
		{
			...
			if (simulationPoint.IsHourly())
			{
				...
				if (variables[H_TMIN] && !data[H_TMIN].is_init() && data[H_TAIR].is_init())
					data.SetStat(H_TMIN, data[H_TAIR][MEAN]);

				if (variables[H_TMAX] && !data[H_TMAX].is_init() && data[H_TAIR].is_init())
					data.SetStat(H_TMAX, data[H_TAIR][MEAN]);
			}
			else
			{
				if (variables[H_TAIR] && !data[H_TAIR].is_init() && data[H_TMIN].is_init() && data[H_TMAX].is_init())
					data.SetStat(H_TAIR, (data[H_TMIN][MEAN] + data[H_TMAX][MEAN]) / 2);
			}

			//if (variables[H_PRES] && !data[H_PRES].is_init())
			//{
			//	data.SetStat(H_PRES, pa / 100);		//pressure [hPa]
			//}

			if (variables[H_TDEW] && !data[H_TDEW].is_init() && data[H_RELH].is_init() && data[H_TAIR].is_init())
			{
				double Td = Hr2Td(data[H_TAIR][MEAN], data[H_RELH][MEAN]);
				data.SetStat(H_TDEW, Td);
			}

			if (variables[H_RELH] && !data[H_RELH].is_init() && data[H_TDEW].is_init() && data[H_TAIR].is_init())
			{
			...
			//WndS from Wnd2
			if (variables[H_WNDS] && !data[H_WNDS].is_init() && data[H_WND2].is_init())
			{
		...
		if (msg && (bTdew || bSRad))
		{
			...
			for (size_t y = 0; y != simulationPoint.size(); y++)//for all years
			{
				...
				for (size_t m = 0; m != simulationPoint[y].size(); m++)//for all months
				{
					...
					{
						const CWeatherDay& day = (const CWeatherDay&)simulationPoint[y][m][d];
						size_t jd = day.GetTRef().GetDOY();
				...
				for (size_t m = 0; m != simulationPoint[y].size(); m++)//for all months
				{
					for (size_t d = 0; d != simulationPoint[y][m].size(); d++)//for all days
					{
						CWeatherDay& wDay = copy[year][m][d];
						size_t jd = wDay.GetTRef().GetDOY();

						//need temperature to compute hourly Tdew and Hr
						wDay[H_TMIN] = data.s_tmin[jd];
						wDay[H_TMAX] = data.s_tmax[jd];

						if (variables[H_TDEW])// && !wDay[H_TDEW].is_init())
							wDay[H_TDEW] = data.s_tdew[jd];

						if (variables[H_RELH])// && !wDay[H_RELH].is_init())
						{
				...
				for (CTRef TRef = period.begin(); TRef <= period.end(); TRef++)
				{
					...
					if (variables[H_TDEW] && !data[H_TDEW].is_init())
						data.SetStat(H_TDEW, copy[TRef][H_TDEW]);

					if (variables[H_RELH] && !data[H_RELH].is_init())
						data.SetStat(H_RELH, copy[TRef][H_RELH]);

					if (variables[H_SRAD] && !data[H_SRAD].is_init())
	...
	//				CDataInterface& data = (*itR)[d];
	//				//output
	//				if (m_tgi.m_variables[H_EA] && !data[H_EA].is_init())
	//				{
	...
	//
	//					double Ea = data[H_EA].is_init() ? data[H_EA][MEAN] : e°(data[H_TDEW][MEAN]);
	//					double Es = data[H_ES].is_init() ? data[H_ES][MEAN] : e°(data[H_TMIN][MEAN], data[H_TMAX][MEAN]);
	//
			...
			for (CWeatherYears::iterator itY = simulationPoint.begin(); itY != simulationPoint.end(); itY++)//for all years
			{
				for (CWeatherYear::iterator itM = itY->second->begin(); itM != itY->second->end(); itM++)//for all months
				{
					for (CMonth::iterator itD = itM->begin(); itD != itM->end(); itD++, jd++)//for all days
					{
						...
						if (variables[H_SNOW] && !(*itD)[H_SNOW].is_init())
							(*itD)[H_SNOW] = snow[jd].m_newSWE;

						if (variables[H_SNDH] && !(*itD)[H_SNDH].is_init())
							(*itD)[H_SNDH] = snow[jd].m_hs;

						if (variables[H_SWE] && !(*itD)[H_SWE].is_init())
	...
	ERMsg CWeatherGenerator::GetHourly(CSimulationPoint& simulationPoint, CCallback& callback)
	{
		...
		if (!((CLocation&)simulationPoint).is_init())
					...
					if (neededVariables[v] && (v != H_TMIN && v != H_TMAX))
					{
						...
						if (!msg)
						{
							...
							if (!vars.empty())
							{
								...
								if (m_tgi.UseGribs())
									msg.ajoute(FormatMsg("The variable \"%1%\" is missing in database:\n\t%2%\nto assess one of these variables:\n\t%3%", GetVariableTitle(v), m_tgi.m_gribsDBName, vars));
							}
						}
				...
				for (TVarH v = H_FIRST_VAR; v < NB_VAR_H && msg; v++)
				{
					if (neededVariables[v] && v != H_TAIR)
					{
						...
						if (!msg)
						{
							...
							{
								if (m_tgi.UseDaily())
									msg.ajoute(FormatMsg("The variable \"%1%\" is missing in database:\n\t%2%\nto assess one of these variables:\n\t%3%", GetVariableTitle(v), m_tgi.m_dailyDBName, vars));
								if (m_tgi.UseGribs())
									msg.ajoute(FormatMsg("The variable \"%1%\" is missing in database:\n\t%2%\nto assess one of these variables:\n\t%3%", GetVariableTitle(v), m_tgi.m_gribsDBName, vars));
							}
								...
								for (TVarH v = H_FIRST_VAR; v < NB_VAR_H; v++)
								{
									if (mVariables[v])
									{
										...
										else
										{
											...
											if (!simulationPointVector[r][y][m][d][v].is_init() && annualData[year][m][d][v].is_init())
			...
			for (TVarH v = H_FIRST_VAR; v < NB_VAR_H; v++)
			{
				if (mVariables[v])
				{
					...
					else
					{
						...
						if (!vars.empty())
							msg.ajoute(FormatMsg("The variable \"%1%\" is missing in database:\n\t%2%\nto assess one of these variables:\n\t%3%", GetVariableTitle(v), GetFileName(m_pNormalDB->GetFilePath()), vars));
					}
				}
			}
		}
	...
	int CWeatherGenerator::Sol9(double inlatit, double inelev, double inslope, double inazimuth, double* expin)
	{
		...
		for (month = 0; month < 12; ++month)
		{
			julian_day = midpnt[month]+1; //+1, for base 1
