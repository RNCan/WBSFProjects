...
namespace WBSF
{
	...
	ERMsg CWeatherAccumulator::Add(const std::vector<std::string>& data, const CWeatherFormat& format)
	{
		ERMsg msg;
	...
	void CWeatherDay::ComputeHourlyVariables(CWVariables variables, std::string options)
	{
		...
		/*std::vector<std::string> op(options, ",;|");

		map<string, string> ops;
		for (size_t i = 0; i < op.size(); i ++)
		{
			std::vector<std::string> pair(options, "=:");
			assert(pair.size() == 2);
			ops[pair[0]] = pair[1];
		}*/
	...
	ERMsg CWeatherYears::Parse(const std::string& str, double nodata)
	{
		...
		while (msg && getline(stream, line))
		{
			...
			if (!line.empty())
			{
				std::vector<std::string> data = Tokenize(line, " ,;\t\r\n", true);
				if (!data.empty())
				{
			...
			else
			{
				...
				for (CWeatherYearSectionMap::const_iterator it = sectionToLoad.begin(); it != sectionToLoad.end(); it++)
				{
					...
					while (msg && begin < count)
					{
						...
						std::vector<std::string> data = Tokenize(str, " ,;\t\r\n", true, begin, end);
						if (!data.empty())
						{
							CTRef TRef = m_format.GetTRef(data);
