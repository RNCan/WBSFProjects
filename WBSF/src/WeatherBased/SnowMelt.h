...
public:
    ...
    CTRef GetSnowMeltDate(short year)const
    {
        ...
        CTRef end(year, JANUARY, DAY_01);

        for (CTRef d = begin; d >= end&&!firstDay.is_init(); d--)
        {
            if (this->at(d).m_hs > 2)//more than 2 cm
            {
                nbDay++;
...
public:
    ...
    void SetSnowPropModel(size_t in)
    {
        m_snowPropModel = in;
    }
    void SetTmelt(double in)
    {
        m_Tmelt = in;
    }
    void SetC1(double in)
    {
        m_C1 = in;
    }
    void SetC2(double in)
    {
        m_C2 = in;
    }
    void SetTt(double in)
    {
        m_Tt = in;
    }
    void SetTr(double in)
    {
        m_Tr = in;
    }
