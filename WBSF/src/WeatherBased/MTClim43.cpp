...
//{
//	assert(site_lat>= -90 && site_lat<=90);
//	assert(site_lon>= -180 && site_lat<=180);
//	assert(site_elevation>= -110 && site_elevation<=5000);
...
int CMTClim43::pulled_boxcar(double *input, double *output, int n, int w, int w_flag)
{
    ...
    if (ok)
    {
        double* wt= (double*)malloc(w * sizeof(double));
...
void CData::data_free()
{
    ...
    if (s_tmax) free(s_tmax);
    s_tmax = NULL;
    if (s_tmin) free(s_tmin);
    s_tmin = NULL;
    if (s_tday) free(s_tday);
    s_tday = NULL;
    if (s_prcp) free(s_prcp);
    s_prcp = NULL;
    //if (s_Es) free(s_Es); s_Es=NULL;
    //if (s_Ea) free(s_Ea); s_Ea=NULL;
    if (s_srad) free(s_srad);
    s_srad = NULL;
    if (s_dayl) free(s_dayl);
    s_dayl = NULL;
    if (s_swe) free(s_swe);
    s_swe = NULL;
    if (s_tdew) free(s_tdew);
    s_tdew = NULL;
