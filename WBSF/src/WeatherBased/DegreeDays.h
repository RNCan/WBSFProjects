...
public:
    ...
    //usefull method
    double GetTmin(const CWeatherDay& in)const
    {
        return m_overheat.GetTmin(in);
    }
    double GetTmax(const CWeatherDay& in)const
    {
        return m_overheat.GetTmax(in);
    }
    double GetTnTx(const CWeatherDay& in)const
    {
        return (GetTmin(in) + GetTmax(in)) / 2.0;
    }
