...
public:
    ...
    friend std::ostream& operator << (std::ostream& stream, const CNormalsDataDeque& data)
    {
        return data << stream;
    }
    friend std::istream& operator >> (std::istream& stream, CNormalsDataDeque& data)
    {
        return data >> stream;
    }
...
public:
    ...
    virtual const char* GetXMLFlag()const
    {
        return XML_FLAG;
    }
    virtual const char* GetDatabaseExtension()const
    {
        return DATABASE_EXT;
    }
    virtual const char* GetOptimizationExtension()const
    {
        return OPT_EXT;
    }
    virtual const char* GetDataExtension()const
    {
        return DATA_EXT;
    }
    virtual const char* GetHeaderExtension()const
    {
        return META_EXT;
    }
    virtual const CTM	GetDataTM()const
    {
        return DATA_TM;
    }
    virtual const char	GetDBType()const
    {
        return 'N';
    }
    ...
    static std::string GetNormalsDataFilePath(const std::string& filePath)
    {
        return WBSF::GetPath(filePath) + GetNormalsDataFileName(filePath);
    }
    static std::string GetNormalsDataFileName(const std::string& filePath)
    {
        return GetFileTitle(filePath)  + DATA_EXT;
    }
    static std::string GetNormalsHeaderFileName(const std::string& filePath)
    {
        return GetFileTitle(filePath) + META_EXT;
    }

    void SetPeriod(int firstYeat, int lastYear);
    int GetFirstYear()const
    {
        return (!m_zop.GetYears().empty() ? *m_zop.GetYears().begin() : INVLID_YEAR);
    }
    int GetLastYear()const
    {
        return (!m_zop.GetYears().empty() ? *m_zop.GetYears().rbegin() : INVLID_YEAR);
    }
    ...
    std::ostream& operator << (std::ostream& stream)const;
    std::istream& operator >> (std::istream& stream);
    friend std::ostream& operator << (std::ostream& stream, const CNormalsDatabase& data)
    {
        return data << stream;
    }
    friend std::istream& operator >> (std::istream& stream, CNormalsDatabase& data)
    {
        return data >> stream;
    }
