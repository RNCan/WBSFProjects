...
namespace WBSF
{
	...
	{
		std::vector<std::string> columnsHeader = Tokenize(header, separator, true);
		return Set(columnsHeader);
	}

	void CTemporal::Set(const std::vector<std::string>& columnsHeader)
	{
	...
	namespace WEATHER
	{
		const double ELEV_FACTOR = 100.0;
		const double SHORE_DISTANCE_FACTOR = 1.0;
	}
	...
	namespace HOURLY_DATA
	{
		...
		//static std::vector<std::string> VARIABLES_TITLE;
...
//			return VARIABLES_TITLE[v].c_str();
			return GetVariableName(v, false);
		}

		static std::vector<std::string> VARIABLES_UNITS_STR;
		const char* GetVariableUnits(size_t v)
		{
			assert(v < NB_VAR_H);

			//if (VARIABLES_UNITS_STR.empty())
              //  WEATHER::VARIABLES_UNITS_STR = Tokenize(IDS_STR_WEATHER_VARIABLES_UNITS, "|;");
			//assert(VARIABLES_UNITS_STR.size() == NB_VAR_H);

			return VARIABLES_UNITS[v];
		}
...
//		}
		...
		{
			CWVariables variables;
			std::vector<std::string> fields = Tokenize(header, separator);
	...
	{
		std::vector<std::string> fields = WBSF::Tokenize(header, separator, true);
		return Set(fields, nodata);
	}

	ERMsg CWeatherFormat::Set(const std::vector<std::string>& columnsHeader, double nodata)
	{
		ERMsg msg;
	...
	CTRef CWeatherFormat::GetTRef(const std::vector<std::string>& data)const
	{
			...
			m_dayPos >= 0 && m_dayPos < data.size())
		{
			...
			if (m < 12 && d < WBSF::GetNbDaysPerMonth(year, m) && h <= 24)
