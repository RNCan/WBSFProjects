...
namespace WBSF
{
	...
	*/
	...
	void CWeatherFileSectionIndex::CompileLine(const std::vector<std::string>& data, const CWeatherFormat& format, CCWeatherYearSection& section)
	{
		bool bTmin = false;
		bool bTmax = false;
	...
	void CWeatherFileSectionIndex::CleanUnusedDataFiles(const std::vector<std::string>& fileName)
	{
		for (CWeatherFileSectionIndex::iterator it1 = begin(); it1 != end(); it1++)
		{
			std::vector<std::string>::const_iterator it2 = FindStringExact(fileName, it1->first, false);
