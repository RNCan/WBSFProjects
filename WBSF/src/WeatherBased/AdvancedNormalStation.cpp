...
#include "Basic/Newmat/Regression.h"
#include "WeatherBased/AdvancedNormalStation.h"
...
namespace WBSF
{
	...
	size_t CANStatistic::GetNbDaysPerMonthMin(size_t v, size_t m)const
	{
		...
		return GetNbDaysPerMonth(m_year, m) - CAdvancedNormalStation::GetNbMissPerMonthMax(v);
	}
					...
					for (size_t d = 0; d < data[m].size(); d++)
					{
						for (TVarH v = H_FIRST_VAR; v < NB_VAR_H; v++)
						{
							if (data[m][d][v].is_init())
							{
								...
								case H_WND2:
								{
									assert(data[m][d].GetData(v)[MEAN] >= 0);
				...
				if (station[y].HaveData())
				{
					...
					for (size_t m = 0; m < 12; m++)
					{
						...
						for (size_t v = 0; v < HOURLY_DATA::NB_VAR_H; v++)
						{
							...
							size_t nbMiss = GetNbDaysPerMonth(data.GetTRef().GetYear(), m) - variables[v].first;
							if (nbMiss <= nbMissMax)
							{
								nbYears[m][v]++;
							}
			...
			for (size_t m = 0; m < 12; m++)
			{
				for (size_t v = 0; v < NB_VAR_H; v++)
				{
					...
					if (m_monthStatArray[y][v][m][NB_VALUE] >= nbDayMin)
					{
						...
						size_t  correction = (v == H_PRCP) ? GetNbDaysPerMonth(m_monthStatArray[y].GetYear(), m) : 1;
	...
	void CAdvancedNormalStation::ComputeTemperature(const CWeatherStation& dailyStation)
	{
				...
				if (dailyStation[y].HaveData())
				{
					...
					if (variables[H_TMIN].first >= nbDayMin && variables[H_TMAX].first >= nbDayMin)
					{
						...
						for (size_t d = 0; d < dailyStation[y][m].size(); d++)
						{
							...
							if (dailyStation[TRef][H_TMIN].is_init() && dailyStation[TRef][H_TMAX].is_init())
							{
		...
		for (size_t m = 0; m < 12; m++)
		{
			if (m_dailyStat[H_TMIN][m].is_init() && m_dailyStat[H_TMAX][m].is_init())
			{
				...
				m_minMaxRelation[m] = statMinMax[m][SUM] / sqrt(statMin[m][SUM2] * statMax[m][SUM2]);
