...
namespace WBSF
{
	...
	protected:
		...
		double GetCropFactor(TCrop type, size_t month)
		{
	...
	public:
		...
		static double GetCn(int type){ assert(type >= 0 && type < NB_REF); return CorrectionFactors[type].m_Cn; }
		static double GetCd(int type){ assert(type >= 0 && type < NB_REF); return CorrectionFactors[type].m_Cd; }

		static double GetCnH(int type, bool bDaytime){ assert(type >= 0 && type < NB_REF); return CorrectionFactorsH[type][bDaytime ? 0 : 1].m_Cn; }
		static double GetCdH(int type, bool bDaytime){ assert(type >= 0 && type < NB_REF); return CorrectionFactorsH[type][bDaytime ? 0 : 1].m_Cd; }
