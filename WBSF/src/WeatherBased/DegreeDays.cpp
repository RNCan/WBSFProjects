...
double CDegreeDays::GetDD(const CWeatherDay& in)const
{
    ...
    case DAILY_AVERAGE:
        DD = GetAverageDD(in);
        break;
    case DAILY_AVERAGE_ADJUSTED:
        DD = GetAverageAdjustedDD(in);
        break;
    case SINGLE_TRIANGLE:
        DD = GetTriangleDD(in);
        break;
    case DOUBLE_TRIANGLE:
        DD = GetDoubleTriangleDD(in);
        break;
    case SINGLE_SINE:
        DD = GetSineDD(in);
        break;
    case DOUBLE_SINE:
        DD = GetDoubleSineDD(in);
        break;
    case MODIFIED_ALLEN_WAVE:
        DD = GetModifiedAllenWaveDD(in);
        break;
    case BASKERVILLE_EMIN:
        DD = GetBaskervilleEminDD(in);
        break;
    default:
        assert(false);
    }
...
double CDegreeDays::GetDD(const CWeatherMonth& in, const CTPeriod& p)const
{
    ...
    {
        if (!p.is_init() || p.is_inside(in[d].GetTRef()))
            DD += GetDD(in[d]);
    }
...
double CDegreeDays::GetAverageDD(const CWeatherDay& in)const
{
    ...
    case HORIZONTAL_CUTOFF:
        DD = x1;
        break;
    case INTERMEDIATE_CUTOFF:
        DD = max(0.0, x1 + x2);
        break;
    case VERTICAL_CUTOFF:
        DD = (GetTnTx(in) <= m_upperThreshold) ? x1 : 0;
        break;
    default:
        assert(false);
    }
...
double CDegreeDays::GetAverageAdjustedDD(const CWeatherDay& in)const
{
    ...
    double Tmax = min(GetTmax(in), m_upperThreshold);
    double x1 = max(0.0, (Tmin + Tmax) / 2 - m_lowerThreshold);
    double x2 = min(0.0, m_upperThreshold - GetTmax(in));
    ...
    case INTERMEDIATE_CUTOFF:
        ...
        break;
    case VERTICAL_CUTOFF:
        DD = (Tmin >= m_lowerThreshold && Tmax <= m_upperThreshold) ? x1 : 0;
        break;
    default:
        assert(false);
    }
...
double CDegreeDays::GetDoubleTriangleDD(const CWeatherDay& in)const
{
    ...
    Tmin = GetTmin(nextDay);
    Tmax = GetTmax(nextDay);
...
double CDegreeDays::GetTriangleDD(double Tmin, double Tmax, double Tl, double Tu, size_t cutoffType)
{
    if (Tmin > Tmax)
        Switch(Tmin, Tmax);
...
double CDegreeDays::GetSineDD(double Tmin, double Tmax, double Tl, double Tu, size_t cutoffType)
{
    if (Tmin > Tmax)
        Switch(Tmin, Tmax);
...
double CDegreeDays::GetModifiedAllenWaveDD(double tmin1, double tmax, double tmin2, double threshLow, double threshHigh)
{
    assert(threshLow <= threshHigh);
    assert(tmin1>-999 && tmax>-999 && tmin2>-999);
...
double CDegreeHours::GetDH(const CHourlyData& in)const
{
    ...
    case INTERMEDIATE_CUTOFF:
        ...
        break;
    case VERTICAL_CUTOFF:
        DH = in[H_TAIR] <= m_upperThreshold ? x1 : 0;
        break;
    default:
        assert(false);
    }
