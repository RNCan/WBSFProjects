...
namespace WBSF
{
	...
	public:
		...
		bool TRefIsChanging(CTRef Tref, int shift = 0)const { return GTRef().is_init() && (Tref - shift).as(m_TM) != (m_lastTRef - shift).as(m_TM); }
		CTRef GetTRef()const { return GTRef().as(m_TM); }
		...
		ERMsg Add(const std::vector<std::string>& data, const CWeatherFormat& format);
	...
	public:
		...
		CTPeriod GetEntireTPeriod(CTM TM)const { return CTPeriod(CTRef(m_TRef.GetYear(), m_TRef.GetMonth(), m_TRef.GetDay(), 0, TM), CTRef(m_TRef.GetYear(), m_TRef.GetMonth(), m_TRef.GetDay(), 23, TM)); }
	...
	public:
		...
		size_t size()const { return GetTRef().GetNbDaysPerMonth(); }
		...
		size_t GetNbDays()const { return GetTRef().GetNbDaysPerMonth(); }
		CTM GetTM()const { return CTM(IsHourly() ? CTM::HOURLY : CTM::DAILY); }
		CTPeriod GetEntireTPeriod()const { return GetEntireTPeriod(GetTM()); }
	...
	public:
		...
		//void ComputeTRange();
		const CWeatherDay& GetDay(size_t DOY)const { CDOYRef TRef(GetTRef().GetYear(), DOY); return at(TRef.GetMonth()).at(TRef.GetDay()); }
		CWeatherDay& GetDay(size_t DOY) { CDOYRef TRef(GetTRef().GetYear(), DOY); return at(TRef.GetMonth()).at(TRef.GetDay()); }
