...
namespace WBSF
{
	...
	namespace HOURLY_DATA
	{
		...
		enum TVarEx
		{
			...
			H_PSYC,				//Psychometric Constant [kPa °C-1]
			...
			H_FNCD,				//cloudness function [dimensionless]
		...
		inline bool IsVariable(size_t v){ assert(v == size_t(H_SKIP) || v < size_t(NB_VAR_H)); return v < size_t(NB_VAR_H); }
		inline bool IsUnknown(size_t v){ return v == size_t(H_SKIP); }
	...
	public:
		...
		void Set(const std::vector<std::string>& columnsHeader);
	...
	public:
		...
		ERMsg Set(const std::vector<std::string>& fields, double nodata);
