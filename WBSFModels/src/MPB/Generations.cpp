...
namespace WBSF
{
	...
	{
		assert(size() > 1);
		assert(minOvipDate < 366);
		assert(maxOvipDate < 366);
		...
		assert(medOvipDate >= 0 && medOvipDate < 365);
	...
	{
		assert(devRates.size() == 365 || devRates.size() == 366);
		assert(dayStart >= 0 && dayStart < 366);
		assert(nbGen > 1);
	...
	{
	assert( devRates.size() == 365 || devRates.size() == 366);
	assert( s>=0 || s<NB_GENERATION_STAGES);
	...
	*/
	...
	{
		assert(curY > 0);//never ask the first year
