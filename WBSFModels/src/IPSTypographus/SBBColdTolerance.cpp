...
namespace WBSF
{
	...
	CSBBColdTolerance::CSBBColdTolerance()
	{
		//	Model parameters
		m_RhoG = 0.311;
		m_MuG = -5;
	...
	m_result.clear();
	m_result.resize(weather.GetNbYear());
	m_result[0].m_year = weather[0].GetYear();
	...
	for(int y=0; y<weather.GetNbYear(); y++) //There can be more than 1 year of weather input (actually there IS).
	{
	int firstDay = y==0?T_0:0; //In first year model starts on startingDay. T_0 in equation [7]. Second year, Jan 1 (0)
	for(int d=firstDay; d<weather[y].GetNbDay(); d++) //Loop over days in weather year
	{
	...
	m_result[y] = CMPBCTResult(weather[y].GetYear(), yearTmin, yearPsurv);
	...
	//Microclimate
	double Tmax=T.GetTMax();
	double Tmin=T.GetTMin();
	...
	double Sin    = sin(2*3.14159*(T.GetDOY()/365. -0.25));
	...
	*/
	...
	void CSBBColdTolerance::ExecuteDaily(const CWeatherStation& weather, CSBBSuperCoolingPointStat& stat)
	{
		...
		size_t jd = T_0;
		for (size_t y = 0; y < weather.size(); y++) //There can be more than 1 year of weather input (actually there IS).
		{
			...
			for (size_t d = firstDay; d < weather[y].GetNbDays(); d++) //Loop over days in weather year
			{
				...
				double p1 = max(0.0, min(1.0, (.5 - Ct) / (.5 - m_Lambda0)));   //Equation [9]
				double p3 = max(0.0, min(1.0, (Ct - 0.5) / (m_Lambda1 - 0.5))); //Equation [9]
				double p2 = 1. - (p1 + p3);                           //Equation [9]
				...
				//m_result[jd] = (CMPBCTResult(weather[y].GetYear(), d, Tmin, Tmax, p1, p3, Ct, LT50, Psurv, Pmort));
