...
//          
//
// Descrition: CSBBDevelopmentTable compute daily devlopement rate of IPS Typographus (spruce bark beetle) 
// Reference:
//*****************************************************************************
// 25/04/2015	Rémi Saint-Amant    Update
...
const double CSBBDevelopmentTable::DEFAULT_P[NB_STAGES][NB_PARAMETER]=
{
// p0                  	=   0.07595  
//	p1                  	=  11.89790  
//	p2                  	=   0.45628  
...
double CSBBDevelopmentTable::GetTo(int s)
{
...
CSBBDevelopmentTable::CSBBDevelopmentTable():CBugDevelopmentTable(NB_STAGES, 0, 45, 0.25)
{
...
double CSBBDevelopmentTable::ComputeRate(size_t s, double T)const
{
	double Rt=0;
    ...
    return Min(1, Max(0, Rt)); //This is daily rate, if time step smaller, must be multiplied by time step...
}

double CSBBDevelopmentTable::GetDayLengthFactor(int s, double t, double dayLength)
{
...
//double CSBBDevelopmentTable::GetRate(int s, double t)const
//{
//	return Max(0, Min(1, CBugDevelopmentTable::GetRate(s, t)*GetDayLenghtFactor( t, dayLength)));
//}

const double CSBBDevelopmentTable::V[NB_STAGES]= 
// Egg        L1		  L2		  L3		Pupae	maturation,		 Adult 
{0.1711,	0.1590,		0.1590,		0.1590,		0.1777,	  0.3162,		0.1  };    


double CSBBDevelopmentTable::GetRelativeRate(const CRandomGenerator& RG, int s, int sex)
{
	...
	int sexFactor = (sex==CBug::MALE)?1:-1;
...
{
	ASSERT( e>=0 && e<NB_EQUATIONS);
	return plnorm(RR, 0-CFL::Square(V[e])/2.0, V[e]);
}
