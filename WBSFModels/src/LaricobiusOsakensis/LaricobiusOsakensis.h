...
public:
    ...
    CLaricobiusOsakensis(const CLaricobiusOsakensis& in) :WBSF::CIndividual(in)
    {
        operator=(in);
    }
    ...
    virtual bool NeedOverheating()const override
    {
        return true;
    }

    virtual void Pack(const WBSF::CIndividualPtr& in)override;
    virtual size_t GetNbStages()const override
    {
        return LOF::NB_STAGES;
    }
    virtual WBSF::CIndividualPtr CreateCopy()const override
    {
        return std::make_shared<CLaricobiusOsakensis>(*this);
    }
...
inline CLNFHost* CLaricobiusOsakensis::GetHost()
{
    return static_cast<CLNFHost*>(m_pHost);
}
inline const CLNFHost* CLaricobiusOsakensis::GetHost()const
{
    return static_cast<const CLNFHost*>(m_pHost);
}
inline CLNFStand* CLaricobiusOsakensis::GetStand()
{
    assert(m_pHost);
    return static_cast<CLNFStand*>(GetHost()->GetStand());
}
inline const CLNFStand* CLaricobiusOsakensis::GetStand()const
{
    assert(m_pHost);
    return static_cast<const CLNFStand*>(GetHost()->GetStand());
}
inline const CLaricobiusOsakensisEquations& CLaricobiusOsakensis::Equations()const
{
    return GetStand()->m_equations;
}
