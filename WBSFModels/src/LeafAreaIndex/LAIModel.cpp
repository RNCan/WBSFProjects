...
namespace WBSF
{
		...
		for (size_t y = 0; y < m_weather.size(); y++)
		{
			for (size_t m = 0; m < m_weather[y].size(); m++)
			{
				for (size_t d = 0; d < m_weather[y][m].size(); d++)
				{
					...
					double LAI = CLeafAreaIndex::ComputeLAI(ref.GetDOY(), m_forestCoverType, m_quantile);
	...
	CStatistic CLAIModel::GetLAI(const CWeatherMonth& weather, size_t forestCoverType, double quantile)
	{
		...
		{
			double LAI = CLeafAreaIndex::ComputeLAI(weather.GetTRef().GetDOY(), forestCoverType, quantile);
			stat += LAI;
		}
