	...
	ERMsg CTemperatureIndexModel::OnExecuteDaily()
	{
		...
		for (size_t y = 0; y < m_weather.GetNbYears(); y++)
		{
			...
			for (CTRef TRef = p.begin(); TRef<=p.end(); TRef++)
			{
				...
				m_output[TRef][O_TMIN_NORMALS] = Normals[TRef.GetDOY()][0];
				m_output[TRef][O_TMIN_NORMALS_SD] = Normals[TRef.GetDOY()][1];
				m_output[TRef][O_TMIN_SCORE] = (data[H_TMIN][MEAN] - Normals[TRef.GetDOY()][0]) / Normals[TRef.GetDOY()][1];
				m_output[TRef][O_TMAX] = data[H_TMAX][MEAN];
				m_output[TRef][O_TMAX_NORMALS] = Normals[TRef.GetDOY()][2];
				m_output[TRef][O_TMAX_NORMALS_SD] = Normals[TRef.GetDOY()][3];
				m_output[TRef][O_TMAX_SCORE] = (data[H_TMAX][MEAN] - Normals[TRef.GetDOY()][2]) / Normals[TRef.GetDOY()][3];
			}
		}
