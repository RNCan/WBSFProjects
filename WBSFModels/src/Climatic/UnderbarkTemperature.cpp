...
namespace WBSF
{
	...
	CMountainPineUnderbarkT::CMountainPineUnderbarkT(const CWeatherDay& weather)
	{
	...
	double CMountainPineUnderbarkT::GetTbark(size_t h, size_t hourTmax)const
	{
		double OH = 0;
	...
	CNewtonianAshUnderbarkT::CNewtonianAshUnderbarkT(double Tair, double K):
		m_K(K),
		m_Tt(0)
	{
	...
	double CNewtonianAshUnderbarkT::GetTbark()const
	{
		assert(m_Tt > -60 && m_Tt < 60);
		return m_Tt;
	}

	double CNewtonianAshUnderbarkT::next_step(double Tair)
	{
		assert(m_Tt > -60 && m_Tt < 60);
