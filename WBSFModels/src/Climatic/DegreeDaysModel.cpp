...
//
//*****************************************************************************
// 25/09/2025	3.3.0	Rémi Saint-Amant    Compile cross-platform
...
namespace WBSF
{
	...
	void CDegreeDaysModel::ComputeFinal(CTM TM, const CModelStatVector& input, CModelStatVector& output)
	{
		...
		CTPeriod pIn = p;
		if (m_firstDate!=CMonthDay(JANUARY, DAY_01) || m_lastDate != CMonthDay(DECEMBER, DAY_31))
			pIn = CTPeriod(CTRef(p.begin().GetYear(), m_firstDate.m_month, m_firstDate.m_day, m_firstDate.m_hour, p.TM()), CTRef(p.end().GetYear(), m_lastDate.m_month, m_lastDate.m_day, m_lastDate.m_hour, p.TM()));
		...
		for (CTRef TRef = stats.m_period.begin(); TRef <= stats.m_period.end(); TRef++)
		{
			...
			if (TRef >= m_firstDate && TRef <= m_lastDate)
			{
				...
				{
					if (stats(TRef, CDegreeDays::S_DD).is_init())
