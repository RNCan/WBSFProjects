//*****************************************************************************
// Class: CMeteorusTrachynotusModel
//
// Description: CMeteorusTrachynotusModel is a BioSIM model of MeteorusTrachynotus
//*****************************************************************************
// 25/02/2020	1.0.0	Rémi Saint-Amant	Creation
//*****************************************************************************

#include "Basic/UtilStd.h"
#include "WeatherBased/SnowAnalysis.h"
#include "ModelBased/EntryPoint.h"
#include "MeteorusTrachynotusModel.h"
...
namespace WBSF
{
	...
	static const bool bRegistred =
		CModelFactory::RegisterModel(CMeteorusTrachynotusModel::CreateObject);
	...
	CMeteorusTrachynotusModel::CMeteorusTrachynotusModel()
	{
	...
	ERMsg CMeteorusTrachynotusModel::ProcessParameters(const CParameterVector& parameters)
	{
		ERMsg msg;
	...
	ERMsg CMeteorusTrachynotusModel::OnExecuteDaily()
	{
		ERMsg msg;
		...
		ExecuteDailyAllGenerations(SBWStat, MeteorusTrachynotusStat);
	...
	void CMeteorusTrachynotusModel::ExecuteDailyAllGenerations(CModelStatVector& /*SBWStat*/, vector<CModelStatVector>& stat)
	{
		...
		for (size_t y = 0; y < m_weather.size(); y++)
		{
			...
			CInitialPopulation initialPopulation(p.begin(), 0, 1000, 100, PUPA, FEMALE, true, 0);


			//Create stand
			CMeteorusTrachynotusStand stand(this);
			...
			pHost->Initialize<CMeteorusTrachynotus>(initialPopulation);
	...
	//************************************************************************************************
	ERMsg CMeteorusTrachynotusModel::OnExecuteAnnual()
	{
		ERMsg msg;
		...
		if (maxG > 0)
		{
			for (CTRef TRef = p.begin(); TRef <= p.end(); TRef++)
			{
				...
				for (size_t g = 1; g < maxG; g++)
				{
					...
					CStatistic diapauseStat = MeteorusTrachynotusStat[g].GetStat(M_DIAPAUSE, season);
					...
					if (g == maxG - 1)
					{
						...
						double diapauseEnd = MeteorusTrachynotusStat[g][season.End()][size_t(m_diapauseAge)];
	...
	//************************************************************************************************
	ERMsg CMeteorusTrachynotusModel::OnExecuteAtemporal()
	{
		ERMsg msg;
		...
		//			CStatistic diapauseStat = MeteorusTrachynotusStat[g].GetStat(E_DIAPAUSE, season);
	...
	//simulated annaling
	//void CMeteorusTrachynotusModel::AddDailyResult(const StringVector& header, const StringVector& data)
	//{
	...
	//}
	//
	//void CMeteorusTrachynotusModel::GetFValueDaily(CStatisticXY& stat)
	//{
	//	ERMsg msg;
