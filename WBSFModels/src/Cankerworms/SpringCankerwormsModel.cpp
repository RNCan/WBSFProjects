...
b9                  	=   0.74483
	...
	void CSpringCankerwormsModel::AddDailyResult(const std::vector<std::string>& header, const std::vector<std::string>& data)
	{
				...
				/*	CModelStatVector statSim1;
					...
					for (size_t i = 0; i < m_SAResult.size(); i++)
					{
						...
						for (size_t p = S_PUPA_ADULT; p <= S_ADULT_EGG; p++)
						{
							...
							if (obsS > 0 &&
								obsS < 100 &&
								statSim1.is_inside(m_SAResult[i].m_ref))
							{
								size_t pp = CSpringCankerwormsCR1::O_FIRST_STAGE + p + 1;

								int year = m_SAResult[i].m_ref.GetYear();
								size_t index = statSim1.GetFirstIndex(pp, obsS, 1, CTPeriod(year, JANUARY, DAY_01, year, DECEMBER, DAY_31));
								if (index >= 1)
								{
					...
					for (size_t i = 0; i < m_SAResult.size(); i++)
					{
						...
						for (size_t p = S_EGG_L1; p <= S_SOIL_PUPA; p++)
						{
							...
							if (obsS > 0 &&
								obsS < 100 &&
								statSim2.is_inside(m_SAResult[i].m_ref))
							{
								size_t pp = CSpringCankerwormsCR2::O_FIRST_STAGE + p + 1;

								int year = m_SAResult[i].m_ref.GetYear();
								size_t index = statSim2.GetFirstIndex(pp, obsS, 1, CTPeriod(year, JANUARY, DAY_01, year, DECEMBER, DAY_31));
								if (index >= 1)
								{
